export const seoPrompt = (payload: {
  userInput: string;
  domain: string;
  output: string[];
}) => {
  const prompt = [
    {
      role: "system",
      content: `You are a professional SEO content writer and optimization expert. Your task is to write a ${payload.domain}-focused article based on the provided userInput. Ensure the content is SEO-optimized, engaging, and tailored to the audience's needs at the given stage. `,
    },
    {
      role: "user",
    ...
    },
    {
      role: "user",
      content: `[IMPORTANT]
            1. Write a ${payload.domain}-focused article that is engaging. \n\n
    ...
    {
      role: "user",
      content: `Do not include any explanations, only provide a RFC8259 compliant JSON response following this format without deviation. {${payload.output
        .map((item) => `"${item}": ""`)
        .join(",\n")}}`,
    },
  ];
